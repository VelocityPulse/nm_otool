...
/*   Updated: 2018/04/16 17:32:00 by cchameyr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		help_print(t_data *nm_data, t_nmlist32 *list, int type)
{
	if (type == N_UNDF)
		type = 'U';
	else if (type == N_ABS)
	...
	{
		ft_printf("         %c ", type, list->str);
		print_by_security_endl(nm_data, list->str);
	}
	else if ((type != 'U' && type != 'u') && !nm_data->u)
	{
		ft_printf("%08llx %c ", nm_bsp32(nm_data, list->ptr->n_value), type);
		print_by_security_endl(nm_data, list->str);
	}
	return (_SUCCESS_);
}
...
int				print_output32(t_data *nm_data)
{
	...
	while (list)
	{
		if (!help_print(nm_data, list, list->ptr->n_type & N_TYPE))
			return (_ERROR_);
		list = list->next;
	}
	return (_SUCCESS_);
